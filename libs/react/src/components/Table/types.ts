...
export interface ITableHeader extends Pick<ITableColumn, 'name' | 'label' | 'sortDirection' | 'forwardRef' | 'fixedLeft' | 'fixedRight'> {
	index: number;
	isLast: boolean;
	isResizing: boolean;
...
export interface ITableState<T> {
	columns: Pick<ITableColumn, 'name' | 'label' | 'sortDirection' | 'width' | 'align' | 'fixedLeft' | 'fixedRight'>[];
	rows?: T[];
	loading: boolean;
	total: number;
	rowHeight: number;
...
export type ModelKey<Item> = Item & ITableRowStatus & IWithId;

export type CustomRenderer<Item> = (props: RenderProps<Item>) => ReactElement | undefined; // Must be or contain a fixed-data-table-2/Cell
export type CustomRenderers<Item> = Record<string, CustomRenderer<Item>>;
